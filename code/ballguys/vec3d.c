...
char *Vec3d_toStringSafe(Vec3d *self, char *buffer, int max)
{
    if (
        max == 0)
    {
        // assuming max 10 digits before decimal,
        // looking at the format string including the constant parts we can determine a good default is calculated as
        // 3 + 10+1+3 + 4 + 10+1+3 + 4 + 10+1+3 + 1 + 1 (\0)
        max = 55;
    }
    snprintf(buffer, max, "{x:%.3f, y:%.3f, z:%.3f}", self->x, self->y, self->z);
    return buffer;
}
...
int Vec3d_validate(Vec3d *self)
{
    ...
    // check for infinity
    if (isinf(self->x) || isinf(self->y) || isinf(self->z))
    {
        return 0;
    }
...
{
    fprintf(dest, "{x:%.3f, y:%.3f, z:%.3f}", self->x, self->y, self->z);
}
